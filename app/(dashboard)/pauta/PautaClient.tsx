...
export function PautaClient({
  ...
  const totals = useMemo(
    () =>
      filtered.reduce(
        (acc, e) => ({
          inversion: acc.inversion + Number(e.ad_spend),
          registros: acc.registros + e.registros_performance,
          asistentes: acc.asistentes + e.asistentes_performance,
          qmAgend: acc.qmAgend + e.qm_agendada_pauta,
          qmAsist: acc.qmAsist + e.qm_asistida_pauta,
          demo: acc.demo + e.demo_pauta,
          won: acc.won + e.won_pauta,
          mrr: acc.mrr + Number(e.mrr_won_pauta),
        }),
        { inversion: 0, registros: 0, asistentes: 0, qmAgend: 0, qmAsist: 0, demo: 0, won: 0, mrr: 0 }
      ),
    [filtered]
  );

  const cpl = totals.registros > 0 ? totals.inversion / totals.registros : 0;
  const tasaAsis = totals.registros > 0 ? Math.round((totals.asistentes / totals.registros) * 100) : 0;
  const eventosConSpend = filtered.filter((e) => Number(e.ad_spend) > 0).length;
  ...
  return (
    <div>
      ...
      <div className="text-muted" style={{ fontSize: 11, marginBottom: 24, padding: "8px 12px", background: "var(--bg-secondary)", borderRadius: 8 }}>
        QMs/Demos/Won de pauta — atribución directa por <code>utm_id</code> en deal. Eventos pre-2026-04-26 pueden estar incompletos (sin backfill de deals históricos).
      </div>
