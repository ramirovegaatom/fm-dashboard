...
export const METRIC_LINEAGE: Record<string, LineageEntry> = {
  ...
  cohorte_entregadas: {
    ...
    column: "COUNT(empresa) por semana de evento_fecha (fecha del ÚLTIMO evento/campaña de la empresa)",
    filter: "toda empresa taggeada a un evento visible (campana_evento); una empresa cuenta UNA vez aunque tenga varios tags — cae en la semana de su evento más reciente",
    update: "Sync Attio phase=tc (cron 30 min / botón) + fechas de evento (Calendario / Third party)",
    note: "José 2026-09-03: la semana es la del evento, sin ningún atributo adicional (antes: fecha de entrada a PRE-QM, que es de la empresa y no del evento). La fecha sale del evento mapeado; si la campaña no tiene fecha mapeada se toma la del nombre de la campaña (Evento_X_DD/MM/YY), marcada con ≈. Región = territorio del último evento (evento → país → nombre).",
  },
  ...
  cohorte_descartadas: {
    ...
    update: "Sync Attio phase=tc (cron 30 min / botón)",
    note: "RECYCLE y Lost NO cuentan como descalificadas. Desde 2026-09-03 el reporte no muestra 'Por procesar' ni 'Otros' (pedido José): esas empresas están dentro de Entregadas, por eso las etapas no suman Entregadas.",
  },
  cohorte_wons: {
