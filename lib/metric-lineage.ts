...
export const METRIC_LINEAGE: Record<string, LineageEntry> = {
  ...
  qm_por_fm: {
    ...
    flow: ATTIO_COMPANIES_FLOW,
    table: "fm_attio_companies",
    column: "outbound_stage IN ('QM AGENDADA', 'QM SHOW', 'QM NO SHOW') · O proceso_fm = 'QM'",
    update: "Cron 4h",
    note: "Empresas marcadas como QM por FM (a nivel Company en Attio). QM NO SHOW se incluye desde 2026-05-27 (pedido Jose) — se agendó la QM aunque no asistieron, sigue siendo QM. FIX 2026-07-08 (Jose): PRE-QM - Oportunidad Marketing ya NO cuenta como QM (era falso positivo, inflaba el conteo).",
  },
  qm_totales: {
    label: "QM Totales",
    flow: DERIVED_FLOW,
    ...
    note: "Total de empresas QM del evento: suma de QM directas + QM influenciadas (partición exacta = qm_por_fm). FIX 2026-07-08 (Jose): la clasificación directa/influenciada ahora se computa por ORIGEN del deal, no por hasOpenDeal (que estaba invertido). Ver qm_generada/qm_influenciada.",
  },
  qm_show: {
    label: "QM Show",
    flow: ATTIO_COMPANIES_FLOW,
  ...
  qm_generada: {
    label: "QM Directa",
    flow: ATTIO_COMPANIES_FLOW,
    table: "fm_pipeline_by_event (join fm_attio_deals)",
    column: "empresa QM AND (deal con origen_negocio IN ('Evento Presencial MKT','Webinars MKT') OR sin deal)",
    filter: "origen del negocio = field marketing (evento presencial / webinar), o QM sin deal asociado",
    update: "Cron 4h",
    note: "QM Directa = FM generó la QM directamente (origen del negocio es un canal de marketing). FIX 2026-07-08 (Jose): antes se deducía por hasOpenDeal (columna qm_type), lo que daba resultados invertidos (7 vs 105 reales). Ahora se computa por el origen del deal en la vista SQL. QM sin ningún deal se cuenta como directa (vino matcheada a un evento FM). La columna qm_type quedó deprecada.",
  },
  qm_influenciada: {
    label: "QM Influenciada",
    flow: ATTIO_COMPANIES_FLOW,
    table: "fm_pipeline_by_event (join fm_attio_deals)",
    column: "empresa QM AND tiene deals pero NINGUNO con origen field marketing",
    filter: "origen del negocio distinto de field marketing (Outbound, Partner, Inbound, etc.)",
    update: "Cron 4h",
    note: "QM Influenciada = la empresa llegó por otro origen pero FM influyó (asistió a un evento). FIX 2026-07-08 (Jose): se computa por origen del deal en la vista SQL, no por qm_type (deprecado).",
  },
  descalificadas: {
